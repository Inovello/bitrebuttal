...
- **Host reboot / process death.** Progress is lost, or the download simply never restarts. Bit Rebuttal runs as an OS service (systemd on Linux, Task Scheduler on Windows) that resumes byte-exact from aria2's `.aria2` control files with no user action.
...
Every binary is also the full CLI: `BitRebuttal-windows-x64.exe add <url>`, `BitRebuttal-linux-x64 status`, and so on.
